...
@Component({
  selector: 'fuse-controller-timepicker',
  templateUrl: './controller-timepicker.component.html',
  styleUrls: ['./controller-timepicker.component.scss'],
  providers: [
    { provide: MatFormFieldControl, useExisting: ControllerTimePickerComponent },
  ],
})
export class ControllerTimePickerComponent implements MatFormFieldControl<string>, ControlValueAccessor, OnDestroy {
  static nextId = 0;
  ...
  @HostBinding() id = `controller-timepicker-${ControllerTimePickerComponent.nextId++}`;
  controlType = 'controller-timepicker';
  ...
  set value(time: string) {
    ...
    const format = /(([0-1][0-9])|(2[0-3])):[0-5][0-9]:[0-5][0-9]/g;
    const result = format.test(time);
    if (!result) {
      return;
    }
    ...
    if (this._onChange) { this._onChange(time); }
  }
